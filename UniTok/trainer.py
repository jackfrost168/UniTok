...
class Trainer(object):
    ...
    def vq_init(self):
        ...
        dataset_lens = [12101, 10429, 8713, 9922, 2420, 
                        8510, 10217, 11924, 10672, 20033]

        all_data = []
        start = 0
        for length in dataset_lens:
            end = start + length
            all_data.append(original_data.subset(start, end))
            start = end

        # Init a shared expert
        all_data.append(original_data.subset(0, end))
        # data1 = original_data.subset(0, 12101)
        # data2 = original_data.subset(12101, 12101+9922)
        # data3 = original_data.subset(12101+9922, 12101+9922+20033)
        # all_data = [data1, data2, data3]
        for data_id, data in enumerate(all_data):
            print("Initializa data:", data_id)
            original_data = data
            ...
            for batch_idx, data in enumerate(iter_data):
                ...
                print("data size in trainer:", data.size())
                self.model.vq_initialization(data, data_id, self.num_experts)    
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        embs  = [layer.embeddings[0].weight.cpu().detach().numpy() for layer in self.model.rq.vq_layers]

        # embs = []
        # for layer in self.model.rq.vq_layers:
        #     # Each layer now has multiple embeddings, stored in embeddings list
        #     layer_embs = [embedding.weight.cpu().detach().numpy() for embedding in layer.embeddings]
        #     embs.append(layer_embs)

        # embs = [embedding.weight.cpu().detach().numpy() for embedding in layer.embeddings]
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            hsic_loss, _, _, _ = self.model.mi_calibration_loss(data, dense_out)
            print("hsic loss:", hsic_loss)
            loss = 0.05*hsic_loss + 0.95*loss
